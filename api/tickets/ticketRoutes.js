...
const errors = {
    ...
    cantUpdate: "Unable to update the ticket",
    cantDelete: "Unable to delete the ticket"
}
...
router.put("/updateTicket", async(req, res) => {
    ...
    ticketHelper.updateTicket(ticket).then(id => {
        if(id.length > 0) res.status(200).json({ id })
        else res.status(400).json({ message: errors.cantUpdate })
    }).catch(err => {
        res.status(500).json({ err, message: errors.cantUpdate })
    })
});
...
router.delete("/deleteTicket", async(req, res) => {
    ...
    ticketHelper.deleteTicket(id).then(i => {
        if(id.length > 0) res.status(200).json({ id })
        else res.status(400).json({ message: errors.cantDelete })
    }).catch(err => {
        res.status(500).json({ err, message: errors.cantDelete })
    })
})
